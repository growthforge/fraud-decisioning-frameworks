...
export const SIM_ASSUMPTIONS: { k: string; v: string }[] = [
  ...
  { k: 'Scores', v: 'logit-normal, deliberately overlapping. A cleanly separable population would teach the wrong lesson. The three segments are scored by the same model, so the shapes are similar, but the riskier segments also carry structurally higher-scoring legitimate traffic — a new customer looks novel because it IS novel. So they differ in prevalence AND, a little, in where their legitimate traffic sits. The prevalence difference is what drives the segmentation result' },
